...
<table id="myTable" style="margin-left:400px ">
  <tr class="header">
    ...
    <th style="width:80%;">Product Id</th>
    <th style="width:80%;">Date Of Purchase</th>
    <th style="width:80%;">Email</th>
    <th style="width:80%;">Reason For The Return</th>
  </tr>
  <?php
  include('db.php');
  ...
  while($row=mysqli_fetch_array($res)){
    ...
    ?>
    <tr id="<?php echo $row['pid'] ?>" onclick="userinfo(this.id)">
...
<input type="button" class="add-row" value="Add Row">
  ...
  <script>
    function userinfo(id){
      location.href="ticketinfo.php?id="+id;
    }
    ...
    if (td) {
      ...
      if (txtValue.toUpperCase().indexOf(filter) > -1) {
        ...
        tr[i].style.display = "none";
               ...
               $.getJSON('http://localhost/ERP/registration/dashboard/examples/tickets.php', function(jd) {
                  ...
                  for(i=0;i<jd.length;i++){
                    pname=jd[i]['pname'];
                    pid=jd[i]['pid'];
                    dateofpurchase=jd[i]['dateofpurchase'];
                    email=jd[i]['uemail'];
                    reason=jd[i]['reason'];
                    pushNewRow(pname,pid,dateofpurchase,email,reason);
